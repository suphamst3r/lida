...
def spatial_temporal_view_decomposition(
    ...
    if type(video_path) == str:
        ...
        if len(video_frames) > 0:
            video_frames = [torch.from_numpy(frame) if type(frame) == np.ndarray else frame for frame in video_frames]
        all_frame_inds = []
        frame_inds = {}
        for stype in samplers:
            frame_inds[stype] = samplers[stype](len(video_frames), is_train)
            all_frame_inds.append(frame_inds[stype])

        all_frame_inds = np.concatenate(all_frame_inds, 0)
        frame_dict = {idx: video_frames[idx] for idx in np.unique(all_frame_inds)}
